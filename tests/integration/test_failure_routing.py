...
async def test_a_temporary_failure_on_the_first_run_goes_to_the_first_retry_queue(
    ...
    await route_failure(
        broker, EVENT, attempt=0, error=TransientError("шлюз недоступен"), kind=TRANSIENT
    )
...
async def test_the_attempt_number_grows_from_retry_to_retry(broker: Recorder) -> None:
    for attempt in range(MAX_HANDLER_RUNS - 1):
        await route_failure(
            broker, EVENT, attempt=attempt, error=TransientError("ещё раз"), kind=TRANSIENT
        )
...
async def test_a_permanent_failure_skips_the_retries(broker: Recorder) -> None:
    await route_failure(
        broker, EVENT, attempt=0, error=PermanentError("платёж не найден"), kind=PERMANENT
    )
...
async def test_the_event_id_survives_both_the_retry_and_the_dead_letter(
    ...
    await route_failure(broker, EVENT, attempt=0, error=TransientError("раз"), kind=TRANSIENT)
    await route_failure(
        broker, EVENT, attempt=MAX_HANDLER_RUNS - 1, error=TransientError("два"), kind=TRANSIENT
    )
