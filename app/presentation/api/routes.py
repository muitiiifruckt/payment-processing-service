...
@router.post(
    ...
    response_model=PaymentAccepted | PaymentView,
...
async def create(
    ...
    if stored.payment_id != payment.payment_id:
        ...
        # RFC §8.2: повтор отвечает полным представлением. Статус и раньше
        # отдавался фактический, но без processed_at и остальных полей
        # контракта клиенту приходилось идти за ними отдельным GET
