...
#: Сколько раз ждать освобождения занятого платежа. Занятость — не отказ
#: обработки, поэтому прогоны за неё не списываются; но и ждать вечно нельзя.
#: Произведение на первую задержку заведомо больше срока удержания захвата
MAX_BUSY_WAITS = 10
